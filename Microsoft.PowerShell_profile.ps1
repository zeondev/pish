...
Write-Host "To access our new package manager, type 'pishpack' (without the quotes).`n";
...
function pishpack {
    ...
    if ($comamnd -eq "install") {
        ...
        $pkgurl = wget "https://raw.githubusercontent.com/zeondev/pishpackrepo/main/$($pkg).ps1";
...
function Get-AliasList {
    $outputDebug = $args[0];
    $outputDebug2 = $args[1];
    [array]$Files = @(Get-ChildItem $PkgLoc);
    ...
    if ($outputDebug -eq "-outputdebug" -and $outputDebug2 -eq "yes") {
        Write-Host "Done`n" -ForegroundColor Green -NoNewLine;
    }
}
